...
//linxc 12-06-19 +++
#define MSM8x25_D9_MSM_FB_SIZE  0x5FB000       //add linxc 2012-12-21 +++
#define GPIO_D9_LCD_BRDG_RESET_N   85
#define GPIO_D9_LCD_BACKLIGHT_EN   96
#define GPIO_D9_LCD_VDD_3V3_EN  12
//linxc 12-06-19 - - - 
...
static int msm_fb_detect_panel(const char *name)
{
   ...
   if (machine_is_msm7x27a_surf() || machine_is_msm7625a_surf() ||
       ...
       #if 1  //#linxc 12-08-23 +++ 
           if (!strncmp(name, "mipi_cmd_novatek_sharp_qhd", 26))           
                   ret = 0;    
       #else
...
static int mipi_sharp_qhd_rotate_panel(void)
{
   int rotate = 0;
   if (machine_is_msm8625_qrd5() || machine_is_msm7x27a_qrd5a())
       rotate = 1;
...
static struct msm_panel_common_pdata mipi_novatek_sharp_qhd_pdata = {
   .pmic_backlight = mipi_sharp_qhd_set_bl,
   .rotate_panel = mipi_sharp_qhd_rotate_panel,
};

static struct platform_device mipi_dsi_sharp_qhd_panel_device = {
   .name = "mipi_novatek_sharp_qhd",
   .id   = 0,
   .dev  = {
       .platform_data = &mipi_novatek_sharp_qhd_pdata,
   }
};
#endif
...
static struct platform_device *evb_fb_devices[] __initdata = {
   ...
   //linxc 12-06-19 +++  
   &mipi_dsi_sharp_qhd_panel_device,
   //linxc 12-06-19 ---
};
...
void __init msm_msm7627a_allocate_memory_regions(void)
{
   ...
   if (machine_is_msm7625a_surf() || machine_is_msm7625a_ffa())
       fb_size = MSM7x25A_MSM_FB_SIZE;
                  ...
                  || machine_is_msm7x27a_qrd5a() || machine_is_msm8625q_skud())
       fb_size = MSM8x25_MSM_FB_SIZE;
   //linxc 12-06-19 +++
   else if (machine_is_msm8625_qrd5()) 
       fb_size = MSM8x25_D9_MSM_FB_SIZE;
   //linxc 12-06-19 ---
   else
       fb_size = MSM_FB_SIZE;
...
static unsigned d9_mipi_dsi_gpio[] = {
   GPIO_CFG(GPIO_D9_LCD_BRDG_RESET_N, 0, GPIO_CFG_OUTPUT,
           GPIO_CFG_NO_PULL,
           GPIO_CFG_2MA), /* GPIO_D9_LCD_BRDG_RESET_N */
   GPIO_CFG(GPIO_D9_LCD_BACKLIGHT_EN, 0, GPIO_CFG_OUTPUT,
           GPIO_CFG_PULL_UP,
           GPIO_CFG_2MA), /* GPIO_D9_LCD_BACKLIGHT_EN   OUTPUT*/
   GPIO_CFG(GPIO_D9_LCD_BACKLIGHT_EN, 0, GPIO_CFG_INPUT,
           GPIO_CFG_NO_PULL,
           GPIO_CFG_2MA),  /* GPIO_D9_LCD_BACKLIGHT_EN  INPUT*/        
};


static int msm_fb_dsi_client_D9_reset(void)
{
   int rc = 0;
   
      // printk("linxc: msm_fb_dsi_client_D9_reset \n");

   rc = gpio_request(GPIO_D9_LCD_BRDG_RESET_N, "d9_lcd_brdg_reset_n");
   if (rc < 0) {
       pr_err("failed to request d9 lcd brdg reset_n\n");
       return rc;
   }
...
#if 1  //linxc 2012-12-21  +++
static int d9_dsi_gpio_initialized = 0;
...
static int mipi_dsi_panel_d9_power(int on)
{
   int rc = 0;
      
   if (!d9_dsi_gpio_initialized) {
       
       printk("linxc: %s  pmapp_disp_backlight_init for backlight \n", __func__);      
       
       pmapp_disp_backlight_init();
       rc = gpio_request(GPIO_D9_LCD_BACKLIGHT_EN, "d9_gpio_bkl_en");
       if (rc < 0){
           pr_err("Failed to request d9_gpio_bkl_en\n");
           return rc;
       }

       d9_dsi_gpio_initialized = 1;
       
       printk("linxc: %s  mdp_pdata.cont_splash_enabled=%d\n", __func__, mdp_pdata.cont_splash_enabled);       

       if (mdp_pdata.cont_splash_enabled) {
           /*Config backlight pin as input, we don't us it */
           rc = gpio_tlmm_config(d9_mipi_dsi_gpio[2],
                    GPIO_CFG_ENABLE);
           if (rc < 0) {
               pr_err("failed D9 GPIO_BACKLIGHT_EN tlmm config\n");
               goto gpio_fail1;
           }
           rc = gpio_direction_input(GPIO_D9_LCD_BACKLIGHT_EN);
           if (rc < 0) {
               pr_err("failed to enable backlight\n");
               goto gpio_fail1;
           }
           ...
           rc = gpio_tlmm_config(d9_mipi_dsi_gpio[0],
                GPIO_CFG_ENABLE);
           if (rc < 0) {
           ...
           rc = gpio_direction_output(GPIO_D9_LCD_BRDG_RESET_N, 1);    //keep reset high from lk
           if (rc < 0) {
               pr_err("Failed GPIO bridge Reset\n");
               goto gpio_fail2;
           }
   ...
   if (on) {       
       printk("linxc: %s  LCD power on 3.3V 2.85V 1.8V \n", __func__);     

       /*Enable EXT_3.3V regulators*/
       rc = regulator_enable(gpio_reg_3p3v);       //D9  GPIO12 3.3V for [LCM]
       if (rc < 0)
           pr_err("%s: reg enable failed\n", __func__);
       /*Enable EXT_2.85 and 1.8 regulators*/
       rc = regulator_enable(gpio_reg_2p85v);      //D9  GPIO35 2.8V for [CAM]
       if (rc < 0)
           pr_err("%s: reg enable failed\n", __func__);
       rc = regulator_enable(gpio_reg_1p8v);       //D9  GPIO34 1.8V for [LCD and CAM]
       if (rc < 0)
           pr_err("%s: reg enable failed\n", __func__);

       /*Configure LCD Bridge reset*/
       rc = gpio_tlmm_config(d9_mipi_dsi_gpio[0], GPIO_CFG_ENABLE);
       if (rc < 0) {
           pr_err("Failed to enable LCD Bridge reset enable\n");
           gpio_free(GPIO_D9_LCD_BRDG_RESET_N);            
           return rc;
       }

       rc = gpio_direction_output(GPIO_D9_LCD_BRDG_RESET_N, 1);
       if (rc < 0) {
           pr_err("Failed GPIO bridge Reset\n");
           gpio_free(GPIO_D9_LCD_BRDG_RESET_N);
           return rc;
       ...
       printk("linxc: %s  Reset Lcd Panel \n", __func__);      
       /*Toggle Bridge Reset GPIO*/
       gpio_set_value_cansleep(GPIO_D9_LCD_BRDG_RESET_N, 1);       
       msleep(20);
       gpio_set_value_cansleep(GPIO_D9_LCD_BRDG_RESET_N, 0);
       msleep(20);
       gpio_set_value_cansleep(GPIO_D9_LCD_BRDG_RESET_N, 1);
       msleep(20);
       ...
       printk("linxc: %s  LCD power off 3.3V 2.85V 1.8V \n", __func__);            
       rc = regulator_disable(gpio_reg_3p3v);
       if (rc < 0)
...
gpio_fail2:
   gpio_free(GPIO_D9_LCD_BRDG_RESET_N);                
gpio_fail1:
   gpio_free(GPIO_D9_LCD_BACKLIGHT_EN);                
   return rc;  
}
#else
...
static int mipi_dsi_panel_power(int on)
{
   ...
   if (machine_is_msm7627a_qrd1())
       rc = mipi_dsi_panel_qrd1_power(on);
                        ...
                        || machine_is_msm8625_qrd5() || machine_is_msm7x27a_qrd5a())
   #if 1  //add by lf 2012.12.19
       rc = mipi_dsi_panel_d9_power(on);
   #else
...
void msm7x27a_set_display_params(char *prim_panel)
{
   ...
   if (strnlen(msm_fb_pdata.prim_panel_name, PANEL_NAME_MAX_LEN)) {
      if (strncmp((char *)msm_fb_pdata.prim_panel_name,
       "mipi_cmd_novatek_sharp_qhd",
       strnlen("mipi_cmd_novatek_sharp_qhd",
           PANEL_NAME_MAX_LEN)) &&
          strncmp((char *)msm_fb_pdata.prim_panel_name,
       "mipi_video_novatek_sharp_qhd",
       strnlen("mipi_video_novatek_sharp_qhd",
           PANEL_NAME_MAX_LEN)))
...
void __init msm_fb_add_devices(void)
{
   ...
   if (machine_is_msm7627a_evb() || machine_is_msm8625_evb()
                   || machine_is_msm8625_qrd5() || machine_is_msm7x27a_qrd5a()) {
       #if 1//add by lf 2012.12.20
       gpio_reg_3p3v = regulator_get(&mipi_dsi_device.dev,
                               "lcd_vddj");
       if (IS_ERR(gpio_reg_3p3v))
           pr_err("%s:ext_3p3v regulator get failed", __func__);
       #endif
       ...
       if (mdp_pdata.cont_splash_enabled) {
       #if 1//add by lf 2012.12.20         
           rc = regulator_enable(gpio_reg_3p3v);
           if (rc < 0)
               pr_err("%s: reg enable failed\n", __func__);    
       #endif          
           /*Enable EXT_2.85 and 1.8 regulators*/
